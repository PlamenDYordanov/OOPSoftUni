...
import static goldDigger.common.ConstantMessages.SPOT_ADDED;
...
public class ControllerImpl implements Controller {
    ...
    public String inspectSpot(String spotName) {
      ...
      List<Discoverer> discovererAbove45Energy =  this.discovererRepository.getCollection()
                .stream()
                .filter(energy -> energy.getEnergy() > 45)
                .collect(Collectors.toList());
        if (discovererAbove45Energy.isEmpty()) {
        ...
        operation.startOperation(spot, discovererAbove45Energy);
       long excluded = discovererAbove45Energy.stream().filter(discoverer -> discoverer.getEnergy() == 0).count();
       this.spotCount++;
    ...
    public String getStatistics() {
        ...
        for (Discoverer discoverer : this.discovererRepository.getCollection()) {
           ...
           if (discoverer.getMuseum().getExhibits().isEmpty()) {
               output.append(String.format(ConstantMessages.FINAL_DISCOVERER_MUSEUM_EXHIBITS, "None"));
           }else {
               output.append(String.format(ConstantMessages.FINAL_DISCOVERER_MUSEUM_EXHIBITS,
